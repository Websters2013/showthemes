		...
		<td style="text-align:left; vertical-align:middle; border: 1px solid #eee; word-wrap:break-word;"><?php
			...
			/*if ( $show_download_links && is_object( $_product ) && $_product->exists() && $_product->is_downloadable() ) {
				...
				foreach ( $download_files as $download_id => $file ) {
					...
					echo '<small>' . $prefix . ': <a href="' . esc_url( $file['download_url'] ) . '" target="_blank">' . esc_html( $file['name'] ) . '</a></small>';
				}
			}*/
			...
			$documentation_url = get_permalink(get_post_meta($_product->id, '_api_product_documentation', true));
			$faq_url = get_permalink(get_post_meta($_product->id, '_api_faq', true));
			
			if(empty($faq_url)){
				$faq_url = home_url('showthemes-faqs');
			}
			if(empty($documentation_url)){
				$documentation_url = 'https://showthemes.zendesk.com/hc';
			}
			
			echo '<small><a href="' . get_permalink(get_option('woocommerce_myaccount_page_id')) . '">' . __('Download at My Account', 'woocommerce') . '</a></small>';
			echo ' | <small><a href="' . $documentation_url . '">' . __('Guide and Documentation', 'woocommerce') . '</a></small>';
			echo ' | <small><a href="' . $faq_url . '">' . __('Tutorials &amp; FAQ', 'woocommerce') . '</a></small>';
